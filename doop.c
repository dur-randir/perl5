...
Perl_do_trans(pTHX_ SV *sv)
{
    ...
    SV* const map = 
#ifdef USE_ITHREADS
		    /*SvRV*/(PAD_SVl(cPADOP->op_padix));
#else
		    /*SvRV*/(MUTABLE_SV(cSVOP->op_sv));
#endif
    bool use_utf8_fcns = (SvTYPE(map) == SVt_PVAV);
    ...
    if (identical) {
        if (use_utf8_fcns) {
            ...
            return do_trans_count(sv, (OPtrans_map *) SvPVX(map));
        }
    }
    else if (use_utf8_fcns) {
        return do_trans_utf8(sv, (AV *) map);
    }
    else if (flags & (OPpTRANS_SQUASH|OPpTRANS_DELETE|OPpTRANS_COMPLEMENT)) {
        return do_trans_complex(sv, (OPtrans_map *) SvPVX(map));
    }
    else 
        return do_trans_simple(sv, (OPtrans_map *) SvPVX(map));
}
